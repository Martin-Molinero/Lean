    ...
    public class InteractiveBrokersFeeModel : FeeModel
    {
        ...
        public override OrderFee GetOrderFee(OrderFeeContext context)
        {
            ...
            if (order.Type == OrderType.OptionExercise)
            {
                ...
                {
                    return new OrderFee(new CashAmount(0, security.QuoteCurrency.AccountCurrency));
                }
            }
            ...
            switch (security.Type)
            {
                case SecurityType.Forex:
                    ...
                    resultFee = Math.Max(_forexMinimumOrderFee, fee);
                    break;
                case SecurityType.Option:
                    // applying commission function to the order
                    resultFee = _optionsCommissionFunc(order.AbsoluteQuantity, order.Price);
                    break;
                case SecurityType.Future:
                    // currently we treat all futures as USD denominated generic US futures
                    resultFee = order.AbsoluteQuantity * (0.85m + 1.0m);
                    break;
                case SecurityType.Equity:
                    ...
                    resultFee = Math.Abs(tradeFee);
                    break;
            }

            // all other types default to zero fees
            return new OrderFee(new CashAmount(resultFee, security.QuoteCurrency.AccountCurrency));
        }
