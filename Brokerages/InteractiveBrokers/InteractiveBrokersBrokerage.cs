...
namespace QuantConnect.Brokerages.InteractiveBrokers
{
    ...
    public sealed class InteractiveBrokersBrokerage : Brokerage, IDataQueueHandler, IDataQueueUniverseProvider
    {
        ...
        {
            var symbolProperties = _symbolPropertiesDatabase.GetSymbolProperties(symbol.ID.Market, symbol,
                symbol.SecurityType, Currencies.USD);
        ...
        /// </summary>
        private double NormalizePriceToBrokerage(decimal price, Contract contract, string ticker, SymbolProperties properties, decimal? minTick = null)
        {
            var roundedPrice = RoundPrice(price, minTick ?? GetMinTick(contract, ticker));
            roundedPrice *= properties.PriceMagnifier;
        ...
        private IBApi.Order ConvertOrder(Order order, Contract contract, int ibOrderId, bool outsideRth)
        {
            ...
            var symbolProperties = _symbolPropertiesDatabase.GetSymbolProperties(order.Symbol.ID.Market, order.Symbol,
                order.SecurityType, Currencies.USD);
            ...
            if (limitOrder != null)
            {
                ibOrder.LmtPrice = NormalizePriceToBrokerage(limitOrder.LimitPrice, contract, order.Symbol, symbolProperties);
            }
            else if (stopMarketOrder != null)
            {
                ibOrder.AuxPrice = NormalizePriceToBrokerage(stopMarketOrder.StopPrice, contract, order.Symbol, symbolProperties);
            }
            else if (stopLimitOrder != null)
            {
                var minTick = GetMinTick(contract, order.Symbol.Value);
                ibOrder.LmtPrice = NormalizePriceToBrokerage(stopLimitOrder.LimitPrice, contract, order.Symbol, symbolProperties, minTick);
                ibOrder.AuxPrice = NormalizePriceToBrokerage(stopLimitOrder.StopPrice, contract, order.Symbol, symbolProperties, minTick);
            }
            else if (limitIfTouchedOrder != null)
            {
                var minTick = GetMinTick(contract, order.Symbol.Value);
                ibOrder.LmtPrice = NormalizePriceToBrokerage(limitIfTouchedOrder.LimitPrice, contract, order.Symbol, symbolProperties, minTick);
                ibOrder.AuxPrice = NormalizePriceToBrokerage(limitIfTouchedOrder.TriggerPrice, contract, order.Symbol, symbolProperties, minTick);
            }
            ...
            if (symbol.ID.SecurityType.IsOption())
            {
                ...
                if (symbol.ID.SecurityType == SecurityType.FutureOption)
                {
                    ...
                    contract.Strike = NormalizePriceToBrokerage(symbol.ID.StrikePrice, underlyingContract,
                        symbol.Underlying, symbolProperties);
                }
                else
                {
        ...
        private Symbol MapSymbol(Contract contract)
        {
            try
            {
                ...
                if (isFutureOption || securityType == SecurityType.Future)
                {
                    ...
                    // we don't have the Lean FOP symbol yet, we use the underlyings future
                ...
                foreach (var symbol in symbols)
                {
                    lock (_sync)
                    {
                        ...
                        if (!_subscribedSymbols.ContainsKey(symbol))
                        {
                            ...
                            var symbolProperties = _symbolPropertiesDatabase.GetSymbolProperties(subscribeSymbol.ID.Market, subscribeSymbol,
                                subscribeSymbol.SecurityType, Currencies.USD);
            ...
            string dataType)
        {
            ...
            var symbolProperties = _symbolPropertiesDatabase.GetSymbolProperties(request.Symbol.ID.Market, request.Symbol,
                request.Symbol.SecurityType, Currencies.USD);
            var priceMagnifier = symbolProperties.PriceMagnifier;
